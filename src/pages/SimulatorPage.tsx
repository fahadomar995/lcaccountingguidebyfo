...
// ─────────────────────────────────────────
/**
 * Build prioritised practice suggestions from past sessions.
 *
 * Heavier mark allocations (120m, 100m) need more frequent review than
 * lighter ones (60m, 80m), so we weight them up. Each candidate question
 * gets a `priority` score that combines:
 *   - mark weight (heavier = higher base)
 *   - recency (longer since last attempt = higher)
 *   - mastery (lower past % = higher)
    ...
    <div className="mb-6 bg-gradient-to-br from-primary/5 via-card to-card border border-primary/20 rounded-lg p-5 shadow-sm">
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="ml-auto text-[10px] font-mono uppercase tracking-widest text-muted-foreground">
          Weighted by marks · recency · accuracy
        </span>
      </div>
      <p className="text-[11px] text-muted-foreground font-body mb-4">
        100 &amp; 120-mark questions count for more — we surface them more often.
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {suggestions.map((s) => (
          <div
            key={s.q.id}
            className="bg-card border border-border rounded-md p-3 flex flex-col"
          >
            <div className="flex items-center justify-between mb-1">
              <span className="font-mono text-xs text-muted-foreground">
                {s.q.isMock ? "MOCK" : s.q.year} · Q{s.q.questionNumber}
              </span>
              <span
            ...
            <p className="text-[11px] font-body text-muted-foreground leading-snug mt-2 mb-3 flex-1">
              {s.reason}
            </p>
            <div className="flex items-center justify-between gap-2">
              <span className="text-[10px] font-mono text-muted-foreground">
                {s.daysSince === null
                  ? "Never tried"
                  : `${s.daysSince}d ago${s.lastPct !== null ? ` · ${s.lastPct}%` : ""}`}
              </span>
              <Button
                size="sm"
                onClick={() => onStart(s.q)}
                className="h-7 px-2 text-[11px] bg-primary hover:bg-primary/90 text-primary-foreground"
              >
                Start
              </Button>
            </div>
