  ...
  3: [
    {
      ...
      prompt: 'Sort each error into whether it DOES or DOES NOT affect the Trial Balance.',
      itemData: { type: 'sort', data: { buckets: ['Affects TB', 'Does NOT affect TB'], items: [
        { label: 'Casting error in Purchases Day Book', correctBucket: 'Affects TB' },
        { label: 'Sales posted to wrong debtor', correctBucket: 'Does NOT affect TB' },
        { label: 'Omission of an entire transaction', correctBucket: 'Does NOT affect TB' },
        { label: 'Single entry — debit posted, no credit', correctBucket: 'Affects TB' },
        { label: 'Compensating errors (equal & opposite)', correctBucket: 'Does NOT affect TB' },
        { label: 'Wrong addition in a ledger account', correctBucket: 'Affects TB' },
      ] } },
      explanation: 'Errors of omission, commission, principle, original entry, and compensating errors do NOT cause a TB imbalance. Only one-sided errors (single entry, casting errors, extraction errors) affect the TB.',
      sectionLink: '3.1.1',
    },
    {
      id: 'r3-02', chapterId: 3, sectionId: '3.1', type: 'mcq',
      prompt: 'Equipment purchased was debited to the Purchases account. What type of error is this?',
      itemData: { type: 'mcq', data: { options: ['Error of Omission', 'Error of Principle', 'Error of Commission', 'Error of Original Entry'], correctIndex: 1 } },
      explanation: 'An error of principle occurs when an entry is made in the wrong CLASS of account — here, a capital item was treated as revenue expenditure.',
      sectionLink: '3.1.1',
    },
    {
      id: 'r3-03', chapterId: 3, sectionId: '3.1', type: 'match',
      prompt: 'Match each error type to its definition.',
      itemData: { type: 'match', data: { pairs: [
        { term: 'Error of Omission', definition: 'A transaction is completely left out of the books' },
        { term: 'Error of Commission', definition: 'Correct class of account but wrong personal account' },
        { term: 'Error of Principle', definition: 'Entry made in the wrong class of account' },
        { term: 'Compensating Error', definition: 'Two or more errors that cancel each other out' },
      ] } },
      explanation: 'These four error types, plus error of original entry and complete reversal of entries, are the six types that do NOT affect the trial balance.',
      sectionLink: '3.1.1',
    },
    {
      id: 'r3-04', chapterId: 3, sectionId: '3.1', type: 'fill-chip',
      prompt: 'An error of ___ occurs when an amount is entered correctly in the books but the wrong figure is taken to the trial balance.',
      itemData: { type: 'fill-chip', data: { sentence: 'An error of ___ occurs when an amount is entered correctly in the books but the wrong figure is taken to the trial balance.', chips: ['extraction', 'omission', 'principle', 'commission'], correctChip: 'extraction' } },
      explanation: 'An extraction error occurs when figures are incorrectly transferred (extracted) from the ledger to the trial balance.',
      sectionLink: '3.1.1',
    },
    {
      id: 'r3-05', chapterId: 3, sectionId: '3.1', type: 'true-false',
      prompt: 'An error of complete reversal of entries affects the trial balance.',
      itemData: { type: 'true-false', data: { statement: 'An error of complete reversal of entries affects the trial balance.', isTrue: false, followUp: { prompt: 'Why not?', chips: ['Both sides are posted but swapped', 'Only one side is posted', 'The amounts differ'], correctChip: 'Both sides are posted but swapped' } } },
      explanation: 'A complete reversal means the debit and credit are both posted but to the wrong sides. Since both sides still have equal entries, the TB still balances.',
      sectionLink: '3.1.1',
    },
    {
      id: 'r3-06', chapterId: 3, sectionId: '3.1', type: 'order',
      prompt: 'Order the steps for correcting an error in the accounts.',
      itemData: { type: 'order', data: { steps: ['Identify the error type', 'Determine what was recorded incorrectly', 'Reverse the incorrect entry', 'Record the correct entry', 'Check the Suspense account (if applicable)'] } },
      explanation: 'Always identify first, then reverse the wrong entry before posting the correct one. If the error caused a TB imbalance, the Suspense account must also be checked.',
      sectionLink: '3.1.1',
    },
    {
      id: 'r3-07', chapterId: 3, sectionId: '3.1', type: 'mcq',
      prompt: 'Goods sold to Murphy (€500) were posted to the account of Murray. The correcting journal entry would:',
      itemData: { type: 'mcq', data: { options: ['Debit Murphy, Credit Murray', 'Debit Murray, Credit Murphy', 'Debit Sales, Credit Murphy', 'Debit Murphy, Credit Sales'], correctIndex: 0 } },
      explanation: 'Murphy should have been debited (he owes the money). Murray was incorrectly debited, so we credit Murray to reverse and debit Murphy.',
      sectionLink: '3.1.1',
    },
    {
      id: 'r3-08', chapterId: 3, sectionId: '3.1', type: 'fill-chip',
      prompt: 'When a transaction is recorded at €350 instead of the correct €530, this is an error of ___.',
      itemData: { type: 'fill-chip', data: { sentence: 'When a transaction is recorded at €350 instead of the correct €530, this is an error of ___.', chips: ['original entry', 'principle', 'commission', 'omission'], correctChip: 'original entry' } },
      explanation: 'An error of original entry occurs when the wrong amount is entered in the books of original entry (day book/journal) and then posted consistently at the wrong amount.',
      sectionLink: '3.1.1',
    },
    {
      id: 'r3-09', chapterId: 3, sectionId: '3.1', type: 'mcq',
      prompt: 'Which of these errors would cause the trial balance to disagree?',
      itemData: { type: 'mcq', data: { options: ['Error of original entry', 'Casting error in the Sales Day Book', 'Error of commission', 'Compensating errors'], correctIndex: 1 } },
      explanation: 'A casting (addition) error in a day book means only one side is affected — the total posted is wrong. This creates a one-sided error that causes the TB to disagree.',
      sectionLink: '3.1.1',
    },
    {
      id: 'r3-10', chapterId: 3, sectionId: '3.1', type: 'define',
      prompt: 'Define: Error of Principle',
      itemData: { type: 'define', data: { term: 'Error of Principle', modelDefinition: 'An error where a transaction is recorded in the wrong class of account, e.g. capital expenditure recorded as revenue expenditure.' } },
      explanation: 'The key distinction is wrong CLASS of account (e.g. asset vs expense), not wrong account within the same class.',
      sectionLink: '3.1.1',
    },
    {
      id: 'r3-11', chapterId: 3, sectionId: '3.1', type: 'true-false',
      prompt: 'A corrected trial balance will always produce a correct set of final accounts.',
      itemData: { type: 'true-false', data: { statement: 'A corrected trial balance will always produce a correct set of final accounts.', isTrue: false, followUp: { prompt: 'Why not?', chips: ['Errors not affecting TB may still exist', 'The TB is never correct', 'Final accounts ignore the TB'], correctChip: 'Errors not affecting TB may still exist' } } },
      explanation: 'Even after correcting all TB-affecting errors, there may still be errors of omission, principle, commission, original entry, reversal, or compensating errors hidden in the accounts.',
      sectionLink: '3.1.1',
    },
    {
      id: 'r3-12', chapterId: 3, sectionId: '3.1', type: 'mcq',
      prompt: 'Returns inwards of €240 were credited to the Returns Outwards account. By how much does this affect the trial balance?',
      itemData: { type: 'mcq', data: { options: ['€240', '€480', 'It does not affect the TB', '€120'], correctIndex: 2 } },
      explanation: 'Both Returns Inwards and Returns Outwards are on the same side (debit). The debit was posted correctly but the credit went to the wrong account on the same side — this is an error of commission that does NOT affect the TB total.',
      sectionLink: '3.1.1',
    },
  ],

  // ────────────────────────────────────────────
  // Chapter 5 — Suspense Account
  // ────────────────────────────────────────────
  5: [
    {
      id: 'r5-01', chapterId: 5, sectionId: '5.1', type: 'mcq',
      prompt: 'When is a Suspense Account opened?',
      itemData: { type: 'mcq', data: { options: ['When the trial balance does not agree', 'When a bank reconciliation is prepared', 'At the start of every financial year', 'When creditors are overpaid'], correctIndex: 0 } },
      explanation: 'A Suspense Account is opened to temporarily record the difference when the trial balance does not balance, allowing final accounts to be prepared while errors are investigated.',
      sectionLink: '5.1.1',
    },
    {
      id: 'r5-02', chapterId: 5, sectionId: '5.1', type: 'sort',
      prompt: 'Sort each error: does it go through the Suspense Account or is it corrected directly?',
      itemData: { type: 'sort', data: { buckets: ['Through Suspense', 'Direct correction (not Suspense)'], items: [
        { label: 'Single entry — debit posted, no credit', correctBucket: 'Through Suspense' },
        { label: 'Error of omission — whole transaction missing', correctBucket: 'Direct correction (not Suspense)' },
        { label: 'Casting error in Purchases Day Book', correctBucket: 'Through Suspense' },
        { label: 'Error of principle — equipment to purchases', correctBucket: 'Direct correction (not Suspense)' },
        { label: 'Amount posted to wrong side of an account', correctBucket: 'Through Suspense' },
        { label: 'Error of commission — wrong debtor', correctBucket: 'Direct correction (not Suspense)' },
      ] } },
      explanation: 'Only errors that cause the trial balance to disagree go through the Suspense Account. Errors that do NOT affect the TB (omission, principle, commission, original entry, reversal, compensating) are corrected directly.',
      sectionLink: '5.1.1',
    },
    {
      id: 'r5-03', chapterId: 5, sectionId: '5.1', type: 'fill-chip',
      prompt: 'If the credit side of the TB exceeds the debit side by €500, the Suspense Account is opened with a ___ balance of €500.',
      itemData: { type: 'fill-chip', data: { sentence: 'If the credit side of the TB exceeds the debit side by €500, the Suspense Account is opened with a ___ balance of €500.', chips: ['debit', 'credit', 'zero', 'contra'], correctChip: 'debit' } },
      explanation: 'The Suspense Account is placed on the short side. If debits are less than credits, a debit balance is entered in Suspense to force the TB to agree temporarily.',
      sectionLink: '5.1.1',
    },
    {
      id: 'r5-04', chapterId: 5, sectionId: '5.1', type: 'true-false',
      prompt: 'The Suspense Account should appear on the Balance Sheet as a permanent item.',
      itemData: { type: 'true-false', data: { statement: 'The Suspense Account should appear on the Balance Sheet as a permanent item.', isTrue: false, followUp: { prompt: 'Why?', chips: ['It is temporary and should clear to zero', 'It only appears in the P&L', 'It is an off-balance sheet item'], correctChip: 'It is temporary and should clear to zero' } } },
      explanation: 'The Suspense Account is a temporary holding account. Once all errors causing the TB imbalance are found and corrected, it clears to zero and disappears from the accounts.',
      sectionLink: '5.1.1',
    },
    {
      id: 'r5-05', chapterId: 5, sectionId: '5.1', type: 'order',
      prompt: 'Order the steps for dealing with a Suspense Account in an exam question.',
      itemData: { type: 'order', data: { steps: ['Note the original TB difference and which side Suspense is on', 'Identify each error and classify as TB-affecting or not', 'Prepare journal entries for all errors', 'Post TB-affecting corrections to the Suspense Account', 'Verify the Suspense Account clears to zero'] } },
      explanation: 'The Suspense Account must clear to zero. If it does not, you have missed an error or made a posting mistake.',
      sectionLink: '5.1.1',
    },
    {
      id: 'r5-06', chapterId: 5, sectionId: '5.1', type: 'mcq',
      prompt: 'Rent paid (€600) was debited to Insurance. What is the correcting journal entry?',
      itemData: { type: 'mcq', data: { options: ['Dr Rent, Cr Insurance — €600', 'Dr Insurance, Cr Rent — €600', 'Dr Rent, Cr Suspense — €600', 'Dr Suspense, Cr Insurance — €600'], correctIndex: 0 } },
      explanation: 'This is an error of principle (wrong class). Debit Rent (correct account) and Credit Insurance (reverse wrong entry). No Suspense involvement because both sides were posted — the TB still balances.',
      sectionLink: '5.1.1',
    },
    {
      id: 'r5-07', chapterId: 5, sectionId: '5.1', type: 'mcq',
      prompt: 'Sales of €1,800 were posted to the Sales account as €1,080. The Suspense Account has a debit balance. The correction requires:',
      itemData: { type: 'mcq', data: { options: ['Cr Sales €720, Cr Suspense €720', 'Cr Sales €720, Dr Suspense €720', 'Dr Sales €720, Cr Suspense €720', 'Dr Suspense €1,800, Cr Sales €1,800'], correctIndex: 0 } },
      explanation: 'Sales was understated by €720 (1,800-1,080). Credit Sales with €720 to increase it. The debit side was correct (debtor posted at €1,800), so the extra €720 credit reduces the debit Suspense balance.',
      sectionLink: '5.1.1',
    },
    {
      id: 'r5-08', chapterId: 5, sectionId: '5.1', type: 'fill-chip',
      prompt: 'After all corrections, the Suspense Account must have a ___ balance.',
      itemData: { type: 'fill-chip', data: { sentence: 'After all corrections, the Suspense Account must have a ___ balance.', chips: ['zero', 'debit', 'credit', 'carried forward'], correctChip: 'zero' } },
      explanation: 'The whole purpose of the Suspense Account is to hold the TB difference temporarily. Once all one-sided errors are corrected, it must clear to zero.',
      sectionLink: '5.1.1',
    },
    {
      id: 'r5-09', chapterId: 5, sectionId: '5.1', type: 'mcq',
      prompt: 'Purchases of €400 were not posted to the Purchases account (debit side only missing). Suspense has a credit balance. The correction is:',
      itemData: { type: 'mcq', data: { options: ['Dr Purchases €400, Cr Suspense €400', 'Dr Suspense €400, Cr Purchases €400', 'Dr Purchases €400, Cr Bank €400', 'Dr Suspense €400, Cr Bank €400'], correctIndex: 0 } },
      explanation: 'The credit (to Creditors) was posted but the debit to Purchases was missing. Debit Purchases and credit Suspense to clear the difference.',
      sectionLink: '5.1.1',
    },
    {
      id: 'r5-10', chapterId: 5, sectionId: '5.1', type: 'true-false',
      prompt: 'An error of commission requires a Suspense Account entry to correct it.',
      itemData: { type: 'true-false', data: { statement: 'An error of commission requires a Suspense Account entry to correct it.', isTrue: false, followUp: { prompt: 'Why?', chips: ['Commission errors do not affect the TB', 'Commission errors are not real errors', 'Suspense is only for bank errors'], correctChip: 'Commission errors do not affect the TB' } } },
      explanation: 'An error of commission (posting to the wrong account within the same class) does not cause a TB imbalance, so the Suspense Account is not involved in its correction.',
      sectionLink: '5.1.1',
    },
    {
      id: 'r5-11', chapterId: 5, sectionId: '5.1', type: 'define',
      prompt: 'Define: Suspense Account',
      itemData: { type: 'define', data: { term: 'Suspense Account', modelDefinition: 'A temporary account used to record the difference on a trial balance, allowing final accounts to be prepared while errors are being investigated. It clears to zero when all errors are corrected.' } },
      explanation: 'The Suspense Account is always temporary. In the SEC exam, you must show it clearing to zero as proof all errors have been found.',
      sectionLink: '5.1.1',
    },
    {
      id: 'r5-12', chapterId: 5, sectionId: '5.1', type: 'mcq',
      prompt: 'Returns outwards of €360 were debited instead of credited. By how much is the Suspense Account affected?',
      itemData: { type: 'mcq', data: { options: ['€720', '€360', '€180', 'Not affected'], correctIndex: 0 } },
      explanation: 'The entry was posted to the wrong side — a €360 credit is missing AND a €360 wrong debit exists. The total effect is €720 (double the amount). Correction: Cr Returns Outwards €720 (or €360 reversal + €360 correct) and Dr Suspense €720.',
      sectionLink: '5.1.1',
    },
  ],
